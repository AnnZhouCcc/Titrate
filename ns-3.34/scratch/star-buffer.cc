...
bool long_goodput_logging = true;
...
void StatGoodput (ApplicationContainer sourceApps, 
  ...
  if (notCompleted) {
    if (!light_logging) {
      Simulator::Schedule (Seconds (fctStatIntervalSec), &StatGoodput, sourceApps, sinkApps, portBase, flowNum, totalPacketsThr, stream);
    } else {
...
void StatGoodputStartEndOnly (ApplicationContainer sourceApps, 
  ...
  if (notCompleted) {
    if (!light_logging) {
      Simulator::Schedule (Seconds (fctStatIntervalSec), &StatGoodputStartEndOnly, sourceApps, sinkApps, portBase, flowNum, totalPacketsThr, stream);
    } else {
      if (long_goodput_logging) {
        Simulator::Schedule (Seconds (fctStatIntervalSec), &StatGoodputStartEndOnly, sourceApps, sinkApps, portBase, flowNum, totalPacketsThr, stream);
      }
...
void InstallApp (Ptr<Node> source,
  ...
  for (uint16_t appIndex = 0; appIndex < appSettings.size (); appIndex++) {
    ...
    uint64_t appConf = std::stoi(appSettings[appIndex].at(1));
    ...
    if (appType == "Video" || appType == "Ctrl") {
      ...
      if (!light_logging) {
        ...
        Simulator::Schedule (Seconds (fctStatIntervalSec+appStart) + startTime, &StatFct, sourceApps, portBase, flowNum, 
          ascii.CreateFileStream (fctTrFileNameFull.c_str ()));

        std::string gptTrFileNameFull = gptTrFileName + "_app" + std::to_string(appIndex) + ".tr";
        Simulator::Schedule (Seconds (fctStatIntervalSec+appStart) + startTime, &StatGoodput, sourceApps, sinkApps, portBase, flowNum, 0,
      ...
      uint32_t flowSizeInPackets = (appConf/1000000)%1000;
      ...
      if (!light_logging) {
        ...
        std::string gptTrFileNameFull = gptTrFileName + "_app" + std::to_string(appIndex) + ".tr";
        Simulator::Schedule (Seconds (fctStatIntervalSec+appStart+startRangeMs/1000.) + startTime, &StatGoodput, sourceApps, sinkApps, portBase, flowNum, 0,
          ascii.CreateFileStream (gptTrFileNameFull.c_str ()));
      } else {
        if (long_goodput_logging) {
          std::string gptTrFileNameFull = gptTrFileName + "_app" + std::to_string(appIndex) + ".tr";
...
void InvokeToRStatsSinkOnly(Ptr<OutputStreamWrapper> stream, uint32_t BufferSize, uint32_t nPrior, uint32_t bufferAlgorithm){
	double nanodelay = statIntervalSec*1e9;
  int64_t currentNanoSeconds = Simulator::Now().GetNanoSeconds();
  ...
  for (uint32_t port = numpqueues-2; port<numpqueues; port++) {
		Ptr<GenQueueDisc> genDisc = DynamicCast<GenQueueDisc>(pqueues.Get(port));
		double remaining = genDisc->GetRemainingBuffer();
		for (uint32_t priority = 0; priority<nPrior; priority++)  {
			uint32_t qSize = genDisc->GetQueueDiscClass(priority)->GetQueueDisc()->GetNBytes();
	...
	*stream->GetStream() << std::endl;
	...
	Simulator::Schedule(Seconds(statIntervalSec), InvokeToRStatsSinkOnly, stream, BufferSize, nPrior, bufferAlgorithm);
}
...
int main (int argc, char *argv[]) {
  ...
  cmd.AddValue ("adaptiveIncreaseParameter", "Step when increase: +drop/adaptiveIncreaseParameter", adaptiveIncreaseParameter);
  cmd.AddValue ("adaptiveDecreaseParameter", "Step when cdcrease: -minbuffer/adaptiveDecreaseParameter", adaptiveDecreaseParameter);
  ...
  conf += "/" + toStringNoTrailingZeros(std::to_string(dropRateThreshold)) + "_" + std::to_string(monitorlongms) + "_" + toStringNoTrailingZeros(std::to_string(adaptiveIncreaseParameter)) + "_" + toStringNoTrailingZeros(std::to_string(adaptiveDecreaseParameter));
  ...
  torStats = torTraceHelper.CreateFileStream (torOutFile);
	*torStats->GetStream ()
	...
	<< "bufferSizeMB "
	<< "occupiedBufferPct";
	for (int i=0; i<(numSinks+numSendersA+numSendersB); i++) {
  ...
  NodeContainer sendersANodes, sendersBNodes, sinksNodes, bufferNodes;
  ...
  sharedMemory->setUp(bottleneckQueueDiscsCollection.GetN(), nPrior, randomSeed, smallestRTTms, mainRoomNumQueues);
  ...
  if (torstats_sinkonly) {
    Simulator::Schedule(MicroSeconds (10),&InvokeToRStatsSinkOnly,torStats, bufferSize, nPrior, bufferAlgorithm);
  } else {
