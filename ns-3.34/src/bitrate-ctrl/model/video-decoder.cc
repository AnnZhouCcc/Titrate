...
VideoDecoder::VideoDecoder(Time delay_ddl, uint16_t fps, GameClient * game_client, uint16_t port,
    Ptr<OutputStreamWrapper> appTrStream, void (GameClient::*ReplyFrameAck)(uint32_t, Time)) {
    ...
    for (auto pkt : pkts) {
        ...
        for (uint32_t idx = m_lastPlayedFrameId + 1; 
            ...
            *m_appTrStream->GetStream () << Simulator::Now().GetMilliSeconds() << 
                " FlowId " << m_port << " Discard frame " << idx << std::endl;
        ...
        if (this->unplayed_frames[frame_id]->GetDataPktNum()
            == this->unplayed_frames[frame_id]->GetDataPktRcvedNum()) {
                if (frame_id == 0) {
                    ...
                    Time time_now = Simulator::Now();
                    *m_appTrStream->GetStream () << Simulator::Now().GetMilliSeconds() << 
                        " FlowId " << m_port << " Decode frame " << frame_id << std::endl;
                }
                  ...
                  for (uint32_t idx = frame_id; idx <= this->max_frame_id; idx ++) {
                        if (this->unplayed_frames.find(idx) != this->unplayed_frames.end()){
                            if(this->unplayed_frames[idx]->GetDataPktNum() == this->unplayed_frames[idx]->GetDataPktRcvedNum()){
                                ...
                                Time time_now = Simulator::Now();
                                *m_appTrStream->GetStream () << Simulator::Now().GetMilliSeconds() << 
                                    " FlowId " << m_port << " Decode frame " << idx << std::endl;                
