#include "packet-udp-receiver.h"
#include "game-client.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE("PacketUdpReceiver");

TypeId PacketUdpReceiver::GetTypeId() {
    static TypeId tid = TypeId ("ns3::PacketUdpReceiver")
...
PacketUdpReceiver::PacketUdpReceiver (uint32_t wndsize, uint16_t interval, Ipv4Address m_peerIP, uint16_t m_peerPort, uint16_t m_localPort)
: PacketReceiver{}
, m_record {}
...
void PacketUdpReceiver::StartApplication(GameClient* client, Ptr<VideoDecoder> decoder, Time delay_ddl, Ptr<Node> node){
    ...
    if (this->m_socket == NULL) {
        ...
        this->m_socket->SetRecvCallback(MakeCallback(&PacketUdpReceiver::OnSocketRecv_receiver,this));
    }
    this->m_feedbackTimer.SetFunction(&PacketUdpReceiver::Feedback_NetState,this);
    this->m_feedbackTimer.SetDelay(m_feedback_interval);
}

void PacketUdpReceiver::OnSocketRecv_receiver(Ptr<Socket> socket)
{
...
void PacketUdpReceiver::ReceiveUdpPacket(Ptr<VideoPacket> pkt) {
...
void PacketUdpReceiver::ReplyACK(std::vector<Ptr<DataPacket>> data_pkts, uint16_t last_pkt_id) {
    ...
    for(auto data_pkt : data_pkts) {
        NS_LOG_FUNCTION ("[Client] At " << Simulator::Now().GetMilliSeconds() << 
            " ms send ACK for group " << data_pkt->GetGroupId() << " pkt " << data_pkt->GetPktIdGroup());
...
void PacketUdpReceiver::Feedback_NetState ()
{
    this->pkts_in_wnd = 0;
    ...
    m_socket->Send(nstpacket->ToNetPacket());
    // this->m_recv_sample.clear();
    m_feedbackTimer.Schedule();
};


void PacketUdpReceiver::SendPacket(Ptr<NetworkPacket> pkt) {
    m_socket->Send(pkt->ToNetPacket());
}

void PacketUdpReceiver::Set_FECgroup_delay(Time& fec_group_delay_)
{
    this->one_way_dispersion = fec_group_delay_;
};

Time PacketUdpReceiver::Get_FECgroup_delay()
{
    return this->one_way_dispersion;
};

void PacketUdpReceiver::OutputStatistics() {
...
void PacketUdpReceiver::StopRunning() {
...
bool PacketUdpReceiver::lessThan(uint16_t id1, uint16_t id2) {
...
bool PacketUdpReceiver::lessThan_simple(uint16_t id1, uint16_t id2){
