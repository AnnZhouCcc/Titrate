...
NS_LOG_COMPONENT_DEFINE ("PacketTcpSender");

NS_OBJECT_ENSURE_REGISTERED (PacketTcpSender);

TypeId PacketTcpSender::GetTypeId (void) {
    static TypeId tid = TypeId ("ns3::PacketTcpSender")
        .SetParent<Object> ()
        .SetGroupName ("bitrate-ctrl")
        .AddConstructor<PacketTcpSender> ()
        .AddAttribute ("SockBufDutyRatio",
            "The target duty ratio of the socket buffer.",
            DoubleValue (0.9),
            MakeDoubleAccessor (&PacketTcpSender::kTargetDutyRatio),
            MakeDoubleChecker<double_t> ())
    ;
    return tid;
};

PacketTcpSender::PacketTcpSender ()
: PacketSender ()
, kDampingCoef {0.5f}
, kProtectDecreaseCoef {0.1f}
{
    NS_LOG_FUNCTION (this);
}

void PacketTcpSender::SetParams (
    ...
    PacketSender::SetParams (gameServer, EncodeFunc, MilliSeconds (40), srcIP, srcPort, dstIP, dstPort, dscp);
...
void PacketTcpSender::StartApplication (Ptr<Node> node) {
    if (m_socket == NULL) {
        ...
        m_socket->SetConnectCallback (
            MakeCallback (&PacketTcpSender::ConnectionSucceeded, this),
            MakeCallback (&PacketTcpSender::ConnectionFailed, this)
        );
    }
    PacketSender::StartApplication(node);
    m_eventUpdateDuty = Simulator::Schedule(MilliSeconds(1), &PacketTcpSender::UpdateDutyRatio, this);
};

void PacketTcpSender::StopRunning(){
    PacketSender::StopRunning();
    m_socket->Close();
    m_eventUpdateDuty.Cancel();
}

void PacketTcpSender::ConnectionSucceeded (Ptr<Socket> socket)
{
...
void PacketTcpSender::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  NS_LOG_LOGIC ("Tcp Connection Failed");
}

void PacketTcpSender::SetController(std::string cca) {
    m_ccEnabled = true;
    m_cc_create = false;
    ...
    NS_LOG_WARN ("[PacketTcpSender] using " << congestionAlgorithmFactory.GetTypeId ().GetName ());
    Ptr<TcpCongestionOps> algo = congestionAlgorithmFactory.Create<TcpCongestionOps> ();
    base->SetCongestionControlAlgorithm (algo);
}

void PacketTcpSender::CreateFrame(uint32_t frame_id, uint16_t pkt_id, uint16_t data_pkt_max_payload, 
    ...
    m_eventSend = Simulator::ScheduleNow(&PacketTcpSender::SendPacket,this);
}

void PacketTcpSender::SendPacket()
{
...
void PacketTcpSender::UpdateDutyRatio () {
...
void PacketTcpSender::UpdateSendingRate () {
    ...
    if (m_firstUpdate) {
        ...
        // printf("[PacketTcpSender] dutyRatio %f txSend %u txThreshold %u nowBuffer %u lastTxBuffer %u sentsize %u "
        ...
        // printf("[PacketTcpSender] Time %u RttNet(ms) %u Bitrate(bps) %u Cwnd (byte) %u Rtt(ms) %u\n", 
        ...
        *m_bwTrStream->GetStream () << Simulator::Now ().GetMilliSeconds () << 
            ...
            " nowBuf " << curPendingBuf <<
            " TcpCongState " << tcpSocket->GetCongState () << std::endl;
        m_lastPendingBuf = curPendingBuf;
    }
};

uint32_t PacketTcpSender::num_frame_in_queue()
{
    return m_queue.size();
};
