...
namespace rmcat {
NS_LOG_COMPONENT_DEFINE ("GccController");
...
const uint16_t kDefaultTrendlineWindowSizeMs = 320; // 20 for 60fps, 8 for 25fps
...
, window_size_(kDefaultTrendlineWindowSizeMs)
...
void GccController::UpdateEstimate(int64_t now_ms) {
  ...
  if (last_fraction_loss_ == 0 && IsInStartPhase(now_ms)) {
    ...
    NS_LOG_FUNCTION ("loss=0 " << delay_based_bitrate_bps_ << " " << current_bitrate_bps_);
    ...
    new_bitrate += 1000;
      ...
      if (loss <= high_loss_threshold_) {
        ...
        if (!has_decreased_since_last_fraction_loss_ &&
          ...
          NS_LOG_FUNCTION ("loss: " << loss << " " << new_bitrate);
        }
      }
    }
...
bool GccController::processFeedback(uint64_t nowUs,
  ...
  NS_LOG_FUNCTION ("@GccController::processFeedback " <<
    " now: " << nowUs <<
    " seq: " << sequence <<
    " rxts: " << rxTimestampUs <<
    " l_inter_arr: " << l_inter_arrival <<
    " l_inter_departure: " << l_inter_departure <<
    " l_inter_delay_var: " << l_inter_delay_var <<
    " l_inter_group_size: " << l_inter_group_size <<
    " l_arrival_time: " << l_arrival_time <<
    " ecn: " << uint32_t (ecn) <<
    " D_hypothesis_: " << D_hypothesis_ <<
    " rate_control_state_: " << rate_control_state_ <<
    " rate_control_region_: " << rate_control_region_ <<
    " current bitrate: " << current_bitrate_bps_);
  const bool res = SenderBasedController::processFeedback(nowUs, sequence,
	...
	if (prev_seq_loss == 0){
		...
		if (delta_seq != 0){
			...
			NS_LOG_FUNCTION ("loss rate every " << LOSS_TIMER << "ms : " << loss_ratio << 
        ", moving_avg : " << loss_moving_avg);
		}
		loss_counter = 0;
	...
	if (!m_lastTimeCalcValid) {
	  m_lastTimeCalcValid = true;
		SetMinMaxBitrate(m_minBw, m_maxBw);
	 	SetSendBitrate(m_initBw, nowUs);
		return true;
  }
  ...
  rtt_ = rtt_us / 1000;
  uint16_t rtt_clamped = rtt_;
  rtt_clamped = std::max(rtt_clamped, (uint16_t) 10);
  rtt_clamped = std::min(rtt_clamped, (uint16_t) 200);

  if (!update_estimate) {
    // Check if it's time for a periodic update or if we should update because
    // of an over-use.
            
		if (last_update_ms_ == -1 || (uint64_t)now_ms - last_update_ms_ > rtt_clamped){
      ...
      uint32_t incoming_rate = std::min((uint32_t)m_RecvR, current_bitrate_bps_); 
        ...
        update_estimate = true;
        NS_LOG_FUNCTION ("incoming_rate: " << incoming_rate << " m_RecvR: " << m_RecvR <<
...
uint32_t GccController::ChangeBitrate(uint32_t new_bitrate_bps,
  ...
  NS_LOG_FUNCTION ("incoming bitrate(bps): " << incoming_bitrate);
  if (incoming_bitrate)
  ...
  switch (rate_control_state_) {
    ...
    case 'I':
      ...
      if (rate_control_region_ == 'N') {
        ...
        NS_LOG_FUNCTION ("case I region N: add " <<additive_increase_bps
          << " new_bitrate_bps " << new_bitrate_bps
          << " incoming bitrate " << incoming_bitrate_bps);
      } else {
        ...
        NS_LOG_FUNCTION ("case I region no N: add "<<multiplicative_increase_bps
          << " new_bitrate_bps " << new_bitrate_bps
          << " incoming bitrate " << incoming_bitrate_bps);
      }
    ...
    case 'D':
      ...
      NS_LOG_FUNCTION ("case D new " << new_bitrate_bps);
      ...
      NS_LOG_FUNCTION ("case D final " << new_bitrate_bps);
      ChangeRegion('N');
      ...
      if (bitrate_is_initialized_ &&
        ...
        NS_LOG_FUNCTION ("decrease: " << last_decrease_);
      }
...
uint32_t GccController::ClampBitrate(uint32_t new_bitrate_bps,
  ...
  const uint32_t max_bitrate_bps = incoming_bitrate_bps ?
      uint32_t (1.5f * incoming_bitrate_bps) + 10000 : 0xffffffff;

  NS_LOG_FUNCTION ("ack rate cap: new bitrate " << new_bitrate_bps << " max bitrate " << max_bitrate_bps);
  new_bitrate_bps = std::min(new_bitrate_bps, max_bitrate_bps);
...
void GccController::OveruseEstimatorUpdate(int64_t t_delta, double ts_delta, int size_delta, char current_hypothesis, int64_t now_ms){
  ...
  NS_LOG_FUNCTION ("t_delta " << t_delta << " ts_delta " << ts_delta);
  ...
  while (delay_hist_.front ().first + window_size_ < now_ms) {
  ...
  const double Eh[2] = {E_[0][0] * h[0] + E_[0][1] * h[1],
 	...
 	const double residual = t_ts_delta - slope_ * h[0] - offset_; // residual is z(t), offset_ is m(t_i), h is H(t_i), slope_ is 1/C(t_i)
...
char GccController::OveruseDetectorDetect(double offset, double ts_delta, int num_of_deltas, int64_t now_ms){
  ...
  NS_LOG_FUNCTION ("offset: " << offset << " T: " << T << " threshold: " << threshold_ <<
    " time_over_using: " << time_over_using_ << " overuse_counter: " << overuse_counter_ << 
    " prev offset: " << D_prev_offset_);
  if (T > threshold_) {
