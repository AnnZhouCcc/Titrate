...
namespace rmcat {
NS_LOG_COMPONENT_DEFINE("SenderBasedController");
...
const float RMCAT_CC_DEFAULT_RINIT = 2500000.; /**< Initial BW in bps: 2.5Mbps -- mimic initial cwnd of 10 pkts */
const float RMCAT_CC_DEFAULT_RMIN = 250000.;  /**< in bps: 250kbps -- 1pkt/frame of 1500B for 25fps */
const float RMCAT_CC_DEFAULT_RMAX = 50000000.; /**< in bps: 50Mbps */
...
bool SenderBasedController::processFeedback(uint64_t nowUs,
    ...
    /* change the packet size according to the intransit queue */
    uint32_t intvl_pseudo_size = 0;
    // std::cout<<"time now: " << nowUs << " sender-based-controoler record size: "<<" packet seq: "
    //          <<packet.sequence<<" last seq: "<<" begin: "<<m_PacketTransitHistory.begin()->sequence<<" end: "<<m_PacketTransitHistory.end()->sequence<<" ";
    for (auto it = m_PacketTransitHistory.begin(); it != m_PacketTransitHistory.end(); it ++) {
        if (it->sequence > m_lastRecvSequence && it->sequence <= packet.sequence &&
            nowUs - it->txTimestampUs < 1000000) {
            intvl_pseudo_size += it->size;
            // std::cout << " [" << it->sequence << " " << it->size << " " << it->txTimestampUs << "] ";
        }
    }
    // std::cout<<std::endl;
    // the in-network queue is not capable of couting packet losses, therefore only pass the loss rate.
    // Meanwhile, since only queue tail packets will be acked, here the size will be adjusted for bw est.
    packet.size = intvl_pseudo_size;
    m_lastRecvSequence = packet.sequence;
    /* change the packet size according to the intransit queue */
...
void SenderBasedController::PrunTransitHistory (uint16_t tar_seq) {
    while (Uint16Less (m_PacketTransitHistory.front().sequence, tar_seq)) {
...
bool SenderBasedController::getCurrentRecvRate(float& rrateBps) {
    ...
    if (!lessThan(firstRxUs, lastRxUs + 1) || timeSpanUs == 0) {
        NS_LOG_ERROR ("SenderBasedController::getCurrentRecvRate,"
                  << " cannot calculate receive rate,"
                  << " all packets were received simultaneously");
        return false;
    }
    ...
    if (pktcnt > m_recvHistory.at(i).size) {
        pktcnt -= m_recvHistory.at(i).size;
    }
    ...
    NS_LOG_FUNCTION ("getCurrentRecvRate:: rrateBps " << rrateBps);
    return true;
}
