...
BulkSendApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BulkSendApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
    .AddAttribute ("TcpCongestionOps",
                   "The TCP congestion control algorithm used by the socket.",
                   TypeIdValue (TcpCubic::GetTypeId ()),
                   MakeTypeIdAccessor (&BulkSendApplication::m_congOps),
                   MakeTypeIdChecker ())
...
{
  NS_LOG_FUNCTION (this << m_totBytes);
  return m_totBytes;
}
  ...
  if (m_tid == TcpSocketFactory::GetTypeId ())
    {
      ...
      ccaFactory.SetTypeId (m_congOps);
      Ptr<TcpCongestionOps> algo = ccaFactory.Create<TcpCongestionOps> ();
      Ptr<TcpSocketBase> tcpSocket = static_cast<TcpSocketBase*> (PeekPointer(m_socket));
      tcpSocket->SetCongestionControlAlgorithm (algo);
    }
...
void BulkSendApplication::SendData (const Address &from, const Address &to)
{
  ...
  while (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    { // Time to send more
      ...
      /* AnnC: Modification/Fix */ 
      bool packetFromUnsent = false;
      /* AnnC: Modification/Fix */
      Ptr<Packet> packet;
      if (m_unsentPacket)
        {
