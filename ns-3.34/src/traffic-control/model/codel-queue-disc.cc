...
{
  NS_LOG_FUNCTION (this);
  uint8_t countIsDroppedByCodel = 0;
  uint8_t countIsDequeuedByCodel = 0;
  ...
  m_delay += (int32_t (ldelay) - m_delay) >> 3;
  if (item && m_useL4s)
    {
      std::cout << "***CAUTION: GenQueueDisc does not handle this case yet with m_useL4s==true" << std::endl;
      uint8_t tosByte = 0;
  ...
  if (item) {
    countIsDequeuedByCodel = countIsDroppedByCodel+1;
  } else {
    countIsDequeuedByCodel = countIsDroppedByCodel;
  }
  item->SetIsDroppedByCodel(countIsDroppedByCodel);
  item->SetIsDequeuedByCodel(countIsDequeuedByCodel);
