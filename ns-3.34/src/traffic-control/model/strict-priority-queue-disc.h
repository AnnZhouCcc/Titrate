 ...
 */
class StrictPriorityQueueDisc : public QueueDisc {
public:
  /**
  ...
  /**
   * \brief StrictPriorityQueueDisc constructor
   *
   * Creates a queue with a depth of 1000 packets per band by default
   */
  StrictPriorityQueueDisc ();

  virtual ~StrictPriorityQueueDisc();
...
private:
  ...
  uint32_t StrictPriorityDropHead (uint32_t incomingBand);
};
