...
{
  uint32_t qSize = StaticCast<DualQCoupledPiSquareQueueDisc> (queue)->GetQueueSize ();
...
main (int argc, char *argv[])
{
  ...
  std::string dualQCoupledPiSquareLinkDataRate = "1.5Mbps";
  std::string dualQCoupledPiSquareLinkDelay = "20ms";
  ...
  Config::SetDefault ("ns3::DualQCoupledPiSquareQueueDisc::QueueLimit", UintegerValue (100));
  ...
  handle = tchDualQCoupledPiSquare.SetRootQueueDisc ("ns3::DualQCoupledPiSquareQueueDisc");
  tchDualQCoupledPiSquare.AddInternalQueues (handle, 2, "ns3::DropTailQueue", "MaxSize", StringValue ("1000p"));
  ...
  queueDiscs = tchDualQCoupledPiSquare.Install (devn2n3);
  ...
  if (writePcap)
    {
      ...
      stmp << pathOut << "/dual-q-coupled-pi-square";
  ...
  if (writeForPlot)
    {
      filePlotQueueDisc << pathOut << "/" << "dual-q-coupled-pi-square-queue-disc.plotme";
      filePlotQueueDiscAvg << pathOut << "/" << "dual-q-coupled-pi-square-queue-disc_avg.plotme";
