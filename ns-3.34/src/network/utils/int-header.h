...
#ifndef SRC_NETWORK_MODEL_INT_HEADER_H_
#define SRC_NETWORK_MODEL_INT_HEADER_H_
...
public:
  ...
  void pushTelemetry(IntHeader::telemetry t){Feedback[n_hops] = t ;} /* Here the assumption is that, before a node pushes feedback info, it first increments n_hops value. Be careful with this. */
  ...
  IntHeader::telemetry getFeedback(uint32_t i){return Feedback[i];}
...
private:
  ...
  IntHeader::telemetry Feedback[16]; // size of 16 is hardcoded for now. Sorry!
//  std::vector<IntHeader::telemetry> Feedback;
};

#endif /* SRC_NETWORK_MODEL_INT_HEADER_H_ */
