...
QueueDiscItem::QueueDiscItem (Ptr<Packet> p, const Address& addr, uint16_t protocol)
  : QueueItem (p),
    ...
    m_protocol (protocol),
    m_txq (0),
    is_dropped_by_codel (0),
    is_dequeued_by_codel (0)
{
...
uint8_t QueueDiscItem::GetIsDroppedByCodel(void) {
  return is_dropped_by_codel;
}

void QueueDiscItem::SetIsDroppedByCodel(uint8_t value) {
  is_dropped_by_codel = value;
}

uint8_t QueueDiscItem::GetIsDequeuedByCodel(void) {
  return is_dequeued_by_codel;
}

void QueueDiscItem::SetIsDequeuedByCodel(uint8_t value) {
  is_dequeued_by_codel = value;
}
