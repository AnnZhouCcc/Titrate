...
void Ipv4Interface::ABCEstimation (Ipv4Header &hdr)
{
  ...
  NS_LOG_DEBUG ("[Ipv4Interface Estimation]" << " time now=" << Simulator::Now().GetMilliSeconds()
...
void Ipv4Interface::SetABCMode(bool flag){
...
Ipv4Interface::Send (Ptr<Packet> p, const Ipv4Header & hdr, Ipv4Address dest)
{
  ...
  Ipv4Header hdr_pkt;
  if (ABCMode){ABCEstimation(hdr_copy); hdr_pkt = hdr_copy;}
  else {hdr_pkt = hdr;}
