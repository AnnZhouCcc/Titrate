...
public:
  ...
  virtual void ProcessIntWien(Ptr<Packet> packet, const TcpHeader& tcpHeader, Ptr<TcpSocketState> tcb){};
  virtual void setTcpWien(){isTcpWien=true;}
  virtual bool getTcpWienEnabled(){return isTcpWien;}
  ...
  virtual IntHeader getReceivedInt(){return receivedInt;}
  virtual void setReceivedInt(IntHeader Int){receivedInt = Int;}

  virtual double getCurrentRate(){return currentRate;}

  // vamsi : For Tcp Wien
    Timer wien_pacingTimer {Timer::CANCEL_ON_DESTROY};
//  Ptr<Timer> wien_pacingTimer;
private:
  bool isTcpWien=false;
  double currentRate=DataRate("1Gbps").GetBitRate();
  std::vector<uint32_t> sentSeq;
  IntHeader receivedInt;
/*Modification*/
};
