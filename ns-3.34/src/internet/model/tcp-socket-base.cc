  ...
  : TcpSocket ()
{
  ...
  /*Modification*/
  wien_pacingTimer.SetFunction(&TcpSocketBase::NotifyPacingPerformed, this);
  m_tcb->m_setWienRateCallback = MakeCallback (&TcpSocketBase::SetWienRate,this);
  /*Modification*/
    ...
    m_ecnCWRSeq (sock.m_ecnCWRSeq)
{
  ...
  if (m_tcb->m_setWienRateCallback.IsNull()){
    m_tcb->m_setWienRateCallback = MakeCallback (&TcpSocketBase::SetWienRate,this);
  }
  /*Modification*/
...
/*Modification*/
void
TcpSocketBase::SetWienRate(Ptr<TcpSocketState> tcb, DataRate rate, DataRate prevRate, Time baseRtt, bool useWindow){

  tcb->wienRate = DataRate(std::max(rate.GetBitRate(), tcb->minWienRate.GetBitRate()));

  if(useWindow){
    tcb->m_cWnd = (std::max((tcb->wienRate.GetBitRate()*baseRtt.GetSeconds())/(8),double(tcb->m_segmentSize)));
  }
  else{
  ...
  if (wien_pacingTimer.IsRunning()){
    ...
    wien_pacingTimer.Cancel();

    Time toSend = tcb->wienRate.CalculateBytesTxTime(lastTimedPacketSize);

    if ( (toSend+left-prevRateTime).GetDouble() > 0 )
      wien_pacingTimer.Schedule(toSend+left-prevRateTime);
    else
      wien_pacingTimer.Schedule(left);
    }
    else
     wien_pacingTimer.Schedule(Seconds(0)); 
}
/*Modification*/
...
TcpSocketBase::GetCongState (void) const
{
  return m_tcb->m_congState;
}
...
TcpSocketBase::GetSockState (void)
{
  return m_state;
}
                                  ...
                                  const uint32_t tcpPayloadSize)
{
  if (tcpHeaderSize == 0 || tcpHeaderSize > 60)
  ...
  else if (tcpPayloadSize > 0 && OutOfRange (seq, seq + tcpPayloadSize))
    {
      ...
      NS_LOG_INFO ("At state " << TcpStateName[m_state] <<
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (m_congestionControl->getTcpWienEnabled() &&  (TcpHeader::ACK==(tcpHeader.GetFlags () & TcpHeader::ACK)) && packet->GetSize()==0){
    m_congestionControl->ProcessIntWien(packet,tcpHeader,m_tcb);
  }
  /*Modification*/
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  /*Modification*/
  if( ( flags == TcpHeader::ACK ) && m_congestionControl->getTcpWienEnabled())
    p->AddPacketTag(m_congestionControl->getReceivedInt());
  /*Modification*/
...
TcpSocketBase::AddSocketTags (const Ptr<Packet> &p) const
{
  ...
  MyPriorityTag a;
  if (p->GetSize() > 70) // considering that less than 70 bytes are all control packets like syn ack etc. 
...
//       std::cout << uint32_t(priority) << std::endl;
  ...
  if (m_congestionControl->getTcpWienEnabled()){
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  /*Modification*/
  if (m_congestionControl->getTcpWienEnabled()){
    if (wien_pacingTimer.IsExpired()){
      wien_pacingTimer.Schedule(m_tcb->wienRate.CalculateBytesTxTime(sz));
  ...
  IntHeader Int;
  bool foundInt = p->PeekPacketTag(Int);
  if(foundInt){
    Int.setPktTimestamp(Simulator::Now().GetNanoSeconds());
...
TcpSocketBase::AdvertisedWindowSize (bool scale) const
{
  ...
  if (w > m_maxWinSize)
    {
      w = m_maxWinSize;
      NS_LOG_INFO ("Adv window size truncated to " << m_maxWinSize << "; possibly to avoid overflow of the 16-bit integer");
    }
...
TcpSocketBase::ReceivedData (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  ...
  if(m_congestionControl->getTcpWienEnabled()){
    IntHeader Int;
    p->PeekPacketTag(Int);
    m_congestionControl->setReceivedInt(Int);
  }
  /*Modification*/
